...
\label{sec:mpill-sos}
The operational semantics of \pill are given as a labelled transition system (LTS), in which processes evolve by performing actions on their free names~\citep{MP-pill}. The semantics follow a Structural Operational Semantics (SOS) style presentation and form the basis for the metatheoretic results of the calculus. We present the multiplicative fragment here; the full LTS is given in~\citet{MP-pill}.

\begin{figure}[p]
  \begin{drules}
  \plto
    {\infer[\ref{rule:mpill-one}]
      {\judge{}{\close{x}{P}}{\cht{x}{\one}}}
      {\deduce
        {\judge{}{P}{\emptyhyp}}
        {\der{D}}}}
  ...
  \infer[\rlabel{\rname{Par\textsubscript{1}}}{rule:mpill-mix-1}]
    {\plto
      {\infer[\ref{rule:mpill-mix}]
  ...
  \infer[\rlabel{\rname{Par\textsubscript{2}}}{rule:mpill-mix-2}]
    {\plto
      {\infer[\ref{rule:mpill-mix}]
        {\judge{}{P \pp Q}{\hyp{G} \pp \hyp{H}}\regh}
        {\deduce
  ...
  \infer[\rlabel{\rname{Syn}}{rule:mpill-mix-sync}]
    {\plto
      {\infer[\ref{rule:mpill-mix}]
        {\judge{}{P \pp Q}{\hyp{G} \pp \hyp{H}}\regh}
        {\deduce
  ...
  \\
  \infer[\rlabel{\rname{$\aleq$}}{rule:mpill-alpha}]
    {\plto
      {\deduce
        {\judge{}{P}{\hyp{G}}}
        {\der{D}}}
    ...
    &\plto
      ...
      {\deduce
        ...
        {\der{E'}}}}
  \quad
  \infer[\rlabel{\rname{\ref*{rule:mpill-one}\ref*{rule:mpill-bot}}}  {rule:mpill-cut-one}]
    {\plto
      {\infer[\ref{rule:mpill-cut}]
    ...
    {\plto
      ...
      {\deduce
        ...
        {\der{D'}}}}
  \\
  \infer[\rlabel{\rname{\ref*{rule:mpill-tensor}\ref*{rule:mpill-parr}}}  {rule:mpill-cut-tensor}]
    {\plto
      {\infer[\ref{rule:mpill-cut}]
    ...
    {\plto
      ...
      {\deduce
        ...
        {\der{D}'}}}
  \\
  \infer[\rlabel{\rname{Res}}{rule:mpill-cut-res}]
    {\plto
      {\infer[\ref{rule:mpill-cut}]
        {\judge{}{\res{xy}{P}}{\hyp{G} \pp \Gamma,\Delta}}
        {\deduce
...
A central metatheoretic result of \pill is \emph{session fidelity}, which states that the LTS of a well-typed process is homomorphic to the LTS induced by its type. Intuitively, a process typed with \hl{$\thl{A tensor B}$} will only ever perform an output action, and its continuation will be typed with \hl{$\thl{B}$}. This guarantees that well-typed processes always communicate according to their protocols. The result is established in~\citet{MP-pill} for the full \pill calculus; we restrict attention to mechanizing the multiplicative fragment in this thesis.


\subsubsection{Operational Semantics for Typinh Environments}



With the operational semantics in place, the remaining challenge for mechanisation is the treatment of name binding. The following section
introduces the locally nameless representation used in the formalisation to handle bound names without reasoning up to $\alpha$-equivalence.
