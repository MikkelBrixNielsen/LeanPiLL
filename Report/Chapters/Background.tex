...
% -------- FIXME: Should there be highlighting on these
%                 Also rewrite / expand
%Transitions are labelled by actions in $\text{Act} = \{, x(), x[y], x(y) \mid x, y \text{ names}\}$, representing empty output, empty input, name output, and name input respectively, together with the silent label $\tau$ for internal communication and parallel composition of labels $l \mid l'$. Communication occurs when two compatible actions are performed over endpoints connected by a restriction producing a $\tau$ transition. This can happen by applying the\cref{rule:mpill-mix-sync,rule:mpill-cut-one,rule:mpill-cut-tensor}.




% -------- FIXME: Example derivation doing steps. Also this should have the ordering of the processes in parallel swapped.

\begin{figure}[t]
  {\def\vlto#1#2{\mspace{6mu}\left\downarrow\vbox to 2.4ex{\vfill\hbox to 84ex{$\mspace{3mu}\scriptstyle #1$\qquad#2\hfill}\vfill}\right.}
  \begin{center}
    \begin{highlight}\small
    ...
    \end{highlight}
  \end{center}
  \caption{FIXME: CAPTION GOES HERE}
...
\begin{example}
  \Cref{fig:-cut-unit-trans} shows a possible transition path for the derivation from~\ref{ex:cut-unit-der}. 
\end{example}


% The derivation path should be one_bot, syn and then for the LHS its should use Typing.one + Typing.mix\_0 and the RHS should use Typing.bot -> Typing.one -> Typing.mix\_0.
