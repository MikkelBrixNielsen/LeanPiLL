...
\begin{example}\label{ex:cut-unit}
  Consider the process \hl{$\phl{P} \defeq \phl{\res{zx}{(\recv{z}{}{\send{y}{}{\nil}} \pp \send{x}{}{\nil})}}$}. This process creates a private session between the restricted endpoints \hl{$\phl{x}$} and \hl{$\phl{z}$}. The right process, \hl{$\phl{\send{x}{}{\nil}}$}, signals termination on \hl{$\phl{x}$} before terminating, while the left process, \hl{$\phl{\recv{z}{}{\send{y}{}{\nil}}}$}, waits for a termination signal on \hl{$\phl{z}$} before proceeding by sending a termination signal on \hl{$\phl{y}$} and then terminating.
\end{example}
...
\begin{example}
  \begin{highlight}
    The process from~\Cref{ex:cut-unit} has the typing $\judge{}{\res{zx}{(\recv{z}{}{\send{y}{}{\nil}} \pp \send{x}{}{\nil})}}{\cht{y}{1}}$, as shown by the derivation below:

    \[
      \infer[\ref{rule:pill-cut}]
        {\judge{}{\res{zx}{(\recv{z}{}{\send{y}{}{\nil}} \pp \send{x}{}{\nil})}}{\cht{y}{1}}}
          {\infer[\ref{rule:pill-mix}]
          {\judge{}{\recv{z}{}{\send{y}{}{\nil}} \pp \send{x}{}{\nil}} {\cht{y}{1}, \cht{z}{\bot} \pp \cht{x}{1}}}
...
Note that, for the typing context \hl{$\thl{\cht{y}{1}, \cht{z}{\bot} \pp \cht{x}{1}}$} to match the premise of~\cref{rule:pill-cut}, we rely on the monoidal properties of environments and hyperenvironments. In particular, using their identity laws, we instantiate the rule's meta-variables as \hl{$\thl{\hyp{G}} = \thl{\emptyhyp}$} and \hl{$\thl{\Delta} = \thl{\emptyseq}$}. Under these instantiations, the premise of the~\ref{rule:pill-cut} rule takes the form \hl{$\thl{\Gamma, \cht{x}{A} \pp \cht{y}{\dual{A}}}$}, which matches the original typing context with \hl{$\thl{\Gamma} = \cht{y}{1}$}. This allows an application of~\cref{rule:pill-cut} over the dual channels \hl{$\phl{x}$} and \hl{$\phl{z}$}.

Since \hl{$\phl{x}$} and \hl{$\phl{z}$} are restricted, they must implement dual session behaviours, reflected in their types by \hl{$\phl{x}$} being assigned \hl{$\thl{1}$} and \hl{$\phl{z}$} being assigned its dual \hl{$\thl{\bot}$}.

Note that, for the typing context \hl{$\thl{\cht{y}{1}, \cht{z}{\bot} \pp \cht{x}{1}}$} to match the premise of~\cref{rule:pill-cut}, we rely on the monoidal properties of environments and hyperenvironments. In particular, using their identity laws, we instantiate the meta-variables of the rule as \hl{$\thl{\hyp{G}} = \thl{\emptyhyp}$} and \hl{$\thl{\Delta} = \thl{\emptyseq}$}. Under these instantiations, the premise of~\cref{rule:pill-cut} takes the form \hl{$\thl{\Gamma, \cht{x}{A} \pp \cht{z}{\dual{A}}}$}, which matches the original typing context with \hl{$\thl{\Gamma} = \cht{y}{1}$}. Since \hl{$\phl{x}$} and \hl{$\phl{z}$} are restricted together, they must implement dual session behaviours, reflected in their types by \hl{$\phl{x}$} being assigned \hl{$\thl{1}$} and \hl{$\phl{z}$} being assigned its dual \hl{$\thl{\bot}$}. Thus, allowing an application of~\cref{rule:pill-cut} over the dual channels \hl{$\phl{x}$} and \hl{$\phl{z}$}.

\subsection{Operational Semantics}
\label{sec:mpill-sos}
The operational semantics of \pill are given as a labelled transition system (LTS) defined over \emph{typing derivations}, in which processes evolve by performing actions on their free names. The semantics follow a Structural Operational Semantics (SOS) style presentation and form the basis for the metatheoretic results of the calculus. We will be focusing on the multiplicative fragment for this part, which is given in \cref{fig:mpill-sos}. The full LTS is given in~\citet{MP-pill}.
