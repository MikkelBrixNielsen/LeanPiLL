...
lemma Proc.lc_le {P : Proc} {k n : Nat} :
  ...
  case one ih | bot ih | tensor ih | parr ih | selectL ih | selectR ih | input ih | server ih
    | duplicate ih | consume ih | dispose ih =>
    simp [Proc.lc] at ⊢ ih
    intro hChan hProc
    constructor
    · exact Channel.lc_le hChan
    · exact ih hProc
  ...
  case link =>
    ...
    intro hx hy
    constructor
    · exact Channel.lc_le hx
    · exact Channel.lc_le hy
...
lemma Typing_buildDup_aux {n : Nat} {QL QR : Proc} {x : FPName} {A : Types}
  ...
  (hlcΓ : Env.lc n Γ) (hlcΓ' : Env.lc n Γ')
  (hNodup_names : names.Nodup) : -- (heq : Γ_acc.names = names.toFinset) :

  n ⊢ wrapDup (#x⟦$N⟧․closeAll (!(#x)⟪x⟫․{QL⟪x⟫}) 1 names.reverse |ₚ !#x․{QR}) names ∷
      [x ∶ !!A ⨂ !!A :: Γ ++ Γ'] := by

  induction names generalizing QL Γ'
  ...
  case cons w ws ih =>
    ...
    have hwΓ:= Env.mem_pair_fst_in_names_iff.mpr hNames.1
