...
@[simp] lemma Types.isServerUsable_shift {d c : Nat} {A : Types} :
  A.isServerUsable ↔ (A ↑ d, c).isServerUsable := by
  cases A <;> simp [HasShift.shift, Types.shift, Types.isServerUsable]
...
@[simp] lemma Types.shiftTypes_substTypes_cancel_0 {A B : Types} :
  B⁺{A // 0} = B := Types.shift_subst_cancel (A := B) (B := A) (d := 0)

-- B{A' // d}{A // i} = B{A ↑ d, 1 // i + 1}{A'{A // i} // d}
