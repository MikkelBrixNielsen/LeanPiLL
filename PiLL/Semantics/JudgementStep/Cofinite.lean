...
inductive TypingStepₘ : {n : Nat} → {𝒢 : HyperEnv} → {P : Proc} → Typing n P 𝒢 →
  ...
  | one_bot
      ...
      {𝒟' : Typing n P' (𝒢 |ₕ [∅‚ Γ])}
      (L_step : Finset FPName)
      (hStep : ∀ x y,
        x ∉ L_step → y ∉ L_step ∪ {x} →
        ∀ (hxL : x ∉ L) (hyL : y ∉ L) (hxy : x ≠ y),
        TypingStepₘ (huniq x hxL y hyL hxy) (x⟦()⟧ |ₗ y⸨()⸩) 𝒟') :
      TypingStepₘ (Typing.cut L huniq) (τ) 𝒟'
  | tensor_parr
      {𝒢 : HyperEnv} {Γ Δ Ξ : Env} {P P' : Proc} {A B : Types} {n : Nat} {L : Finset FPName}
      {huniq : ∀ x ∉ L, ∀ y ∉ L, x ≠ y →
        Typing n (P⸨#x, #y⸩) (𝒢 |ₕ [x ∶ A ⨂ B :: Γ‚ Δ] |ₕ [y ∶ Aᗮ ⅋ Bᗮ :: Ξ])}
      ...
      (hStep : ∀ x y x' y',
        x ∉ L_step → y ∉ L_step ∪ {x} →
        x' ∉ L_step ∪ {x, y} → y' ∉ L_step ∪ {x, y, x'} →
        ∀ (hxL : x ∉ L) (hyL : y ∉ L) (hxy : x ≠ y)
          (hx'L : x' ∉ L) (hy'L : y' ∉ L) (hx'y' : x' ≠ y')
          (hxx' : x ≠ x') (hxy' : x ≠ y') (hyx' : y ≠ x') (hyy' : y ≠ y'),
        TypingStepₘ
          (huniq x hxL y hyL hxy)
          (x⟦x'⟧ |ₗ y⸨y'⸩)
          (huniq' x hxL y hyL hxy x' hx'L y' hy'L hx'y' hxx' hxy' hyx' hyy')) :
      TypingStepₘ
        (Typing.cut L huniq)
        (τ)
